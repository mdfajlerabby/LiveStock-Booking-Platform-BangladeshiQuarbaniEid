...
function TopBreeds() {
  ...
  return <section className="section breeds-section"><div className="container"><div className="section-head"><div><span className="eyebrow">জনপ্রিয় জাত</span><h2>সেরা জাতগুলো</h2></div></div><div className="breed-grid">{breeds.map(([name, description]) => <div className="breed-card" key={name}><span className="breed-number">০{breeds.findIndex((breed) => breed[0] === name) + 1}</span><div><h3>{name}</h3><p className="muted">{description}</p></div></div>)}</div></div></section>;
}

export default function Home() {
  return <><Navbar /><main><div className="container"><Hero /></div><section className="section"><div className="container"><div className="section-head"><div><span className="eyebrow">আজকের বাছাই</span><h2>জনপ্রিয় পশু</h2></div>  <Link href="/animals" className="btn btn-soft">সব দেখুন →</Link></div><div className="animal-grid">{animals.slice(0, 4).map((animal) => <AnimalCard key={animal.id} animal={animal} />)}</div></div></section><Tips /><TopBreeds /></main><Footer /></>;
}
